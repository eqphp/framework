...
class mg{
    ...
    function __construct($collection = '', $database = null){
        try {
            $server = system::config('mongo.server');
    ...
    function post($data, $method = 'batchInsert', $option = array()){
        ...
        $function = $method !== 'save' ? 'insert' : 'save';
        logger::mongo(sprintf('db.%s.%s(%s,%s)', $this->collection, $function, json_encode($data), json_encode($option)), false);
        return $this->collection->$method($data, $option);
    }

    //修改文档
    function patch($data, $condition, $is_all = true, $is_insert = false){
