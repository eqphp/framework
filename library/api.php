...
class api{
    ...
    protected function build_api($api){
        $api = system::config('api.' . $api);
        $service = strstr($api, '.', true);

        $uri = str_replace('.', '/', ltrim($api, $service));
        $service = system::config('api.service.' . $service);
    ...
    protected function parse_code(){
        ...
        if ($this->response_code) {
            if ($this->response_code >= 400) {
                $message = system::config('http_status.' . $this->response_code);
        ...
        throw new Exception('api server response code absent', 113);
    }
    ...
    static function get_user_agent(){
        ...
        $user_agent = 'restful/' . self::VERSION . ' (cURL/';
