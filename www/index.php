...
$server_host=isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'www.eqphp.com';
...
$controller='a_'.url($start_lie);
if ($controller == 'a_' || !class_exists($controller)) {
    $controller='a_index';
}

$method=trim(url($start_lie+1));
if (!method_exists($controller,$method)) if (!method_exists($controller,'__call')) if (!method_exists($controller,'__callStatic')) if (method_exists($controller,'index')) {
    $method='index';
} else {
    http::send(404,0);
