...
class picture{
    ...
    static function code($text, $width = 72, $height = 36){
        ...
        //绘制文本
        for ($i = 0; $i < 5; $i++) {
            imagettftext($img, rand(15, 32), rand(-30, 30), 15 * $i + 3, rand(18, $height - 18) + 10, $color[rand(0, 2)], DATA_STORE . 'font/lucon.ttf', $text[$i]);
        }
    ...
    static function mark($picture, $mark = array('http://127.0.0.1', 'EQPHP'), $file_name = array('test.png', 'file/create/')){
        ...
        imagettftext($image, 9, 0, $img_width - 145, $img_height - 14, $text_color, DATA_STORE . 'font/lucon.ttf', $mark[0]);
        imagettftext($image, 9, 0, $img_width - 210, $img_height - 15, $text_color, DATA_STORE . 'font/msyh.ttf', $mark[1]);
