...
class debug{
    ...
    //开发模式-调试方法
    static function out($param='no param',$mode=1,$is_exit=true){
        header(UTF8);
        list($open,$close)=array('<pre>'.PHP_EOL,'</pre>');
    ...
    static function info($type=1){
        ...
        if (is_int($type) && $type < 7) $type=$data[$type];
        switch($type){
        case 'const':
        ...
        default:
            ...
            return array('system'=>php_uname(),'service'=>php_sapi_name(),
